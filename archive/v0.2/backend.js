...
const audioFiles = [
    ...
    { title: "Phruska Sequence", file: "miaost/Phruska Sequence.mp3", anime: "Made in Abyss: Dawn of the Deep Soul" },
...
function autocompleteSong(input) {
    ...
    const filteredTitles = audioFiles
        .map(track => track.title)
        .filter(title => title.toLowerCase().includes(input.toLowerCase()));
    
    filteredTitles.forEach(title => {
        const item = document.createElement('div');
        item.innerHTML = `<strong>${title.substr(0, input.length)}</strong>${title.substr(input.length)}`;
